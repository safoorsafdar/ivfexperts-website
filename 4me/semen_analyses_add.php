...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_sa'])) {
    ...
    if (empty($patient_id)) {
        ...
        if ($editing > 0) {
            ...
            if ($stmt) {
                $stmt->bind_param("iissssssiddddddddsssdssssssi", $patient_id, $hospital_id, $report_type, $file_path, $lab_name, $lab_report_number, $coll_time, $exam_time, $abstinence, $volume, $ph, $conc, $pr, $np, $im, $norm, $abnorm, $appearance, $liquefaction, $viscosity, $vitality, $round_cells, $debris, $wbc, $agglut, $auto_diag, $notes, $editing);
                if ($stmt->execute()) {
            ...
            if ($stmt) {
                $stmt->bind_param("iisssssssiddddddddsssdssssss", $patient_id, $hospital_id, $qrcode_hash, $report_type, $file_path, $lab_name, $lab_report_number, $coll_time, $exam_time, $abstinence, $volume, $ph, $conc, $pr, $np, $im, $norm, $abnorm, $appearance, $liquefaction, $viscosity, $vitality, $round_cells, $debris, $wbc, $agglut, $auto_diag, $notes);
                if ($stmt->execute()) {
